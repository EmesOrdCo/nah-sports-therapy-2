...
```
contact form → /api/enquiry → Resend → njhsportstherapyandpilates@gmail.com
```
...
- **To** `njhsportstherapyandpilates@gmail.com` — the working inbox, unchanged.
...
`netlify/functions/enquiry.js` validates and sends; `netlify/functions/enquiry-email.js` builds the HTML. That email is tables and inline styles with hex colours on purpose — Outlook renders through Word, and the `oklch()` values the site uses mean nothing to a mail client.
