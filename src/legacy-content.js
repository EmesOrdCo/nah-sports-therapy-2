export const legacyPages = {
  "/pilates": {
    parent: "pilates",
    images: [
      {
        src: "/images/legacy/stott-pilates.gif",
        alt: "STOTT Pilates",
      },
      {
        src: "/images/legacy/pilates-studio-main.jpg",
        alt: "NJH Pilates studio in Studham",
      },
    ],
    headings: [
      "In Studham near Whipsnade",
      "NJH Pilates Treatment",
      "1:1 PILATES",
      "SMALL GROUP PILATES",
      "PRE/POST NATAL PILATES",
      "PILATES FOR GOLFERS",
    ],
    copy: `NJH Pilates
In Studham near Whipsnade

• Improve Posture
• Core Strength & Stability
• Release Unwanted Tension
• Restore Balance to muscles around joint

Awareness is the greatest agent for change.

NJH Pilates Treatment

Pilates uses slow, precise, controlled exercises to strengthen deep postural muscles, in turn stabilising and supporting the body.

The benefits of Pilates are understanding:

• how to correct poor posture
• improve strength and range of movement around the joint
• promotion and maintenance of a healthy back

1:1 PILATES
SMALL GROUP PILATES
PRE/POST NATAL PILATES
PILATES FOR GOLFERS`,
  },
  "/blank-1": {
    parent: "pilates",
    images: [
      {
        src: "/images/legacy/pilates-studio-1.jpg",
        alt: "NJH Pilates studio interior",
      },
      {
        src: "/images/legacy/pilates-studio-main.jpg",
        alt: "Pilates equipment in the Studham studio",
      },
      {
        src: "/images/legacy/pilates-studio-3.jpg",
        alt: "Light and airy Pilates studio",
      },
      {
        src: "/images/legacy/pilates-studio-4.jpg",
        alt: "Treatment and movement space at NJH",
      },
    ],
    headings: ["In Studham, near Whipsnade"],
    copy: `Pilates Studio
In Studham, near Whipsnade

January 2016 saw the launch of my beautiful Sports Therapy and Pilates Studio. This tranquil, light and airy space provides the perfect place to switch off and focus on you.

Whether it is to receive Soft Tissue Release with a Sports Therapy appointment, Individual or Small Group Pilates sessions this is the perfect place to restore your muscular well being and improve your posture.

Please do feel free to give me a call on

07881 821901 to discuss your specific needs.`,
  },
  "/small-group-pilates-timetable": {
    parent: "pilates",
    images: [
      {
        src: "/images/legacy/group-pilates.jpg",
        alt: "Small group Pilates class",
      },
      {
        src: "/images/legacy/pilates-exercise.jpg",
        alt: "Pilates exercise",
      },
    ],
    headings: ["SMALL GROUP PILATES SESSIONS"],
    copy: `PILATES TIMETABLE

SMALL GROUP PILATES SESSIONS

Monday 6.30pm

Tuesday 8.30am

9.20am

11.30am

Friday 7.30am

9.30am

Initial 1:1 assessment including postural analysis is
required prior to joining Small Group Pilates

Class sizes are small to ensure you receive the utmost attention to
correct posture and movement during each exercise where necessary.

Session length 55 minutes

8.30am session (45 minutes)`,
  },
  "/individual-pilates": {
    parent: "pilates",
    headings: ["PILATES SESSIONS DESIGNED FOR YOU"],
    copy: `PRIVATE 1:1 SESSIONS

PILATES SESSIONS DESIGNED FOR YOU

I offer one-to-one Pilates or a
‘duet session’ (one-to-two) with a friend if you prefer.

Both the session format and the exercises given are
tailored to suit your needs and objectives.

Your 1:1 session will focus on restoring your muscular balance, with specific exercises designed for your posture type or medical conditions such as osteoporosis, frozen shoulder, neck, knee or lower back issues.

These sessions allow for greater progression and development and are a must for anyone requiring that individual attention.

Private one hour 1:1 sessions are available Monday to Friday.

For further details or to discuss your specific requirements
please do feel free to contact
Natasha Hadland on 07881 821901.`,
  },
  "/pilates-for-golfers": {
    parent: "pilates",
    images: [
      {
        src: "/images/legacy/golfer-anatomy.png",
        alt: "Anatomical muscles used in golf",
      },
      {
        src: "/images/legacy/golfer-swing.jpeg",
        alt: "Golfer swing sequence",
      },
    ],
    copy: `Pilates for Golfers

For a golfer, muscle imbalances can affect the
legs, hips, arms, shoulders, and the lower back.

Pilates is based on movement from the center of the body,
as are most shots in golf.

Core strength can improve hip rotation, range of motion
in the shoulders and back stability leading to improved performance.

*Improve balance and flexibility
* Strengthen your core to avoid injury
* Work on overall breathing and focus

Individual/Small Group Pilates Sessions or
your own group of friends`,
  },
  "/blank": {
    parent: "pilates",
    images: [
      {
        src: "/images/legacy/prenatal-pilates.jpg",
        alt: "Prenatal Pilates exercise",
      },
      {
        src: "/images/legacy/postnatal-pilates.jpg",
        alt: "Postnatal Pilates exercise",
      },
    ],
    headings: ["PRENATAL PILATES", "POST NATAL PILATES"],
    copy: `Pre/Post Natal Pilates

PRENATAL PILATES

*Maintain fitness during pregnancy
*Improve Posture
*Strengthen Pelvic Floor
*Release Tension

POST NATAL PILATES

After 6 week GP check (8-12 weeks following C section)

*Restore abdominal strength after birth
*Improve medical conditions eg Diastasis Recti

Individual or your own small group of NCT / pre/post natal friends.

Maximum 1:7 to ensure you receive utmost attention and correction when necessary

If you would like more information or
to discuss your specific needs
please do give me a call`,
  },
  "/retreats": {
    parent: "pilates",
    historical: true,
    images: [
      {
        src: "/images/legacy/retreat-arundel.jpeg",
        alt: "Arundel Pilates Retreat poster",
      },
      {
        src: "/images/legacy/retreat-italy.jpeg",
        alt: "Italian Pilates Retreat poster",
      },
    ],
    headings: ["Italian Retreat"],
    copy: `January 2020 Retreat

Scroll down for retreat in Italy in June

Click here for a YouTube video on Brooklands Barn - the stunning location for the Pilates Retreat

Arundel Pilates Retreat, Brooklands Barn, Sussex.
15th-17th January 2020
Beautifully restored 19th Century 10 bedroom barn conversion.
Heated indoor pool, Outdoor Barrel Sauna, pilates studio and treatment rooms.
Fantastic local walks and time to explore Arundel just 1 mile away
Inviting lounge with Inglenook fireplace
Includes 2 nights accommodation, breakfast, lunch & dinner (alcohol not inc)
4 small group pilates sessions run by Natasha.
Soft Tissue appointments available with Natasha at an additional cost.
£395pp Shared room  £430 Single room. (£200 deposit)
For further information please contact Natasha Hadland 07881821901

Italian Retreat

Italian Pilates Retreat Forani Palace, Casperia.
3rd - 7th June 2021
Forani Palace: a 15th century palazzo 90 minutes from Rome Fiumicino airport
Daily small group Pilates sessions run by Natasha are included
En-suite twin and double bedrooms
Breakfast and traditional style 3/4 course Italian dinner with wine served from La Vigna.
With advanced notice GF and other dietary requirements can be catered for.
Fully equipped kitchen with indoor and outdoor dining area
Includes use of beautiful outdoor pool, bikes and a guided walk of the stunning area.
Indoor ‘cave’ pilates studio and outdoor Pilates platform

£695 per person (£200 single occupancy supplement)
Price does not include: Lunch, flights or transfers. All participants must be covered by their own
travel insurance.`,
  },
  "/clinic-policies": {
    parent: "pilates",
    copy: `Clinic Policies

Patients under 16 years of age must be accompanied by a parent who will be required to sign a parental consent form.

In accordance with standard practice we regret that a charge will be made for missed Soft Tissue, 1:1 or 2:1 Pilates appointments or non-attendance unless 24 hours notice is given. A 50% charge is required for less than 48 hours notice.

If you are late for your 1:1 or 2:1 the session still falls within the scheduled appointment time. If the practitioner misses a scheduled 1:1 appointment, you will receive a free session.

Small group Pilates blocks are paid for in termly blocks. Once payment is received in advance of the block your place is reserved for the entire block. Please note no refunds will be given.`,
  },
  "/charity-work": {
    parent: "clinics",
    copy: `Charity Work

NJH Sports Therapy & Pilates is keen to help raise money for charities in the area, giving pre and post event sports massage at events such as triathlons, marathons and 10k runs where possible.

Please contact Natasha if you would like to discuss this further.

Please see link for article featured in the Institute of Sport and Remedial Massage magazine.`,
  },
  "/clinics": {
    parent: "clinics",
    headings: [
      "Berkhamsted Physiotherapy\nand Sports Injury Clinic",
      "Sportspace Berkhamsted",
    ],
    copy: `Clinics - for an appointment call 07881 821 901

Berkhamsted Physiotherapy
and Sports Injury Clinic

36A Lower Kings Road Berkhamsted Herts
HP4 2AA

Location details

Sportspace Berkhamsted

Lagley Meadow
Douglas Gardens
Berkhamsted
Herts
HP4 3QQ

Location Details`,
  },
  "/links": {
    parent: "clinics",
    images: [
      {
        src: "/images/legacy/lssm-mark.gif",
        alt: "London School of Sports Massage",
      },
      {
        src: "/images/legacy/isrm-logo.jpg",
        alt: "Institute of Sport and Remedial Massage",
      },
      {
        src: "/images/legacy/merrithew-logo.png",
        alt: "Merrithew STOTT Pilates",
      },
    ],
    copy: `Links

London School of Sports Massage

Institute of Sport & Remedial Massage

http://www.merrithew.com/stott-pilates/about Merrithew - STOTT Pilates`,
  },
  "/testimonial": {
    parent: "clinics",
    headings: [
      "Ankle injury & Pilates",
      "Pilates",
      "Sports Therapy & Pilates",
      "Back & Shoulders & Pilates",
      "Rotator cuff injury and ankle surgery.",
      "Calf and flat arches",
      "Asthma Sufferer",
      "Ankle ligament sprain from Netball Training",
      "Lower back pain",
      "Pre-Event 'London to Brighton Cycle Ride' Treatment",
      "Lower Back Pain",
    ],
    copy: `Testimonials

Ankle injury & Pilates

'I call Natasha my miracle worker. Not only has she released the muscles in my neck and shoulders so I no longer ache constantly, but she has also given me regained movement in my foot following a severely ruptured tendon. I now attend Natasha's beginners pilates classes and after only a short time can already feel a real difference in my body.' Penny, Marketing Manager

Pilates

' Since meeting and attending the sessions with Natasha I have found her to be dedicated, professional, understanding and attentive. Her small group classes have help me to gain an understanding of my medical condition and how best to manage this. All within a brand new studio, new equipment and a peaceful village location. Rosa, Teaching Assistant, age 50

Sports Therapy & Pilates

'l started pilates with Natasha initially because I had a problem with my hip. After almost 3 months of weekly classes and 3 sports therapy sessions I am amazed at the range of pain free movement I now have. I have also been much more aware of my posture since Natasha has identified and corrected this and feel generally much more balanced. My core strength and general mobility is improving with every session. I can thoroughly recommend Natasha's classes and feel so much better since I have been going.' Sarah, Manager, age 30

Pilates

' Having never done Pilates before, I was a little nervous of what it might entail. However, the small group size and Natasha's clear instructions very much helped, and I would now find it very difficult to miss my weekly class! My general fitness level has really improved. Natasha is an excellent teacher and I would have no hesitation in recommending her. ' Nicola, age 40

Pilates ‘Natasha is the best Pilates teacher I have had over many years - very professional and all exercises are personalised to suit . The facilities are excellent and the classes are very enjoyable . ‘ Jill - ‘ageing golfer '

Back & Shoulders & Pilates

“I have been seeing Natasha for three years now and she has been amazing. Her sports therapy sessions have been a life-saver when my back and shoulders were particularly bad and she is one of the most knowledgeable and professional therapists I have ever visited. More recently I have been lucky enough to be trained by Natasha in pilates (both on a 1-1 basis and in her small group classes) and she is again outstanding. Her attention to detail is second to none and she always manages to suit each exercise to everyone’s individual needs, while ensuring proper discipline in the pilates groundwork. Her classes are challenging but also relaxing in her beautiful new studio. She is remarkably flexible in accommodating makeup classes whenever she can, although we always appreciate that this is her doing us a favour rather than a right! All in all, Natasha is fabulous – my life would be a lot more stressful and painful without the hard work that Natasha has put in to helping me and I can’t thank her enough for her efforts”

Kristina, 35, Investment banker

Sports Therapy & Pilates

'Natasha was recommended to me by a work colleague because I was suffering from lower back pain that began as occasional episodes but had progressed to more chronic in nature. The problem was preventing me from participating in exercise classes, cycling and making a day at my work desk very uncomfortable. A Sports Therapy appointment for Soft Tissue Release provided some initial relief and my postural analysis highlighted how I could improve my posture. Following this I then embarked on a course of pilates with Natasha. Having been someone who always preferred tough circuit classes, I was amazed at the improvement in my lower back following my first 6 or so classes. My lower back problems have more or less vanished and I am now comfortable at work and able to undertake other forms of exercise. Needless to say, I am now a pilates convert and continue to attend classes with Natasha as well as to practice at home some of the exercises she has recommended to me.' Kim, Managing Director age 40

Pilates

'I attend Natasha’s Pilates group. Natasha is a very thoughtful practitioner. She understands her clients’ needs and tailors their Pilates exercises to meet those needs. The classes are small in order that clients can have individual attention. I come away feeling taller, stronger and straighter, sometimes I even have to adjust my rear-view mirror before driving off after a session to accommodate my new found height!

I would recommend Natasha to anyone.'

Tessa, 52, teacher

Rotator cuff injury and ankle surgery.

"I came to Natasha with sports injuries both having had operations. Her treatment hugely helped my long standing shoulder injury, increasing my range and ease of movement.

I am now able to work better in the gym and play tennis without pain. The myofascia work greatly improved my upper body rotation and released the scar tissue on my ankle. Natasha tailored her treatment very specifically to my injuries with good sound anatomical knowledge."

Laura, age 30, Physiotherapist

Calf and flat arches

“I originally contacted Natasha due to on going problems with my calves and hamstrings which was affecting may ability to train for and play recreational sport as the muscles were constantly tight. Natasha quickly diagnosed the problems with my legs and through a variety of massage, fascia, and other innovative sports therapy techniques rectified the injury and reversed the long term underlying problems. This combined with a series of dedicated strengthening, stretching and flexibility exercises has allowed me to train harder than I have been able to for years. Natasha also takes the time to ensure you understand the treatment being performed and its intended outcome. I am pleased to say I am now pain free and I would recommend Natasha to anyone.”

Danny, age 36 Chartered surveyor

Asthma Sufferer

"I suffer from asthma and my breathing is much improved from Natasha's myofascia treatment inparticular."

Anne, age 61 Retired

Ankle ligament sprain from Netball Training

"I could barely put any weight on my foot until Natasha worked on it. The following morning, there was lots more mobility and the swelling was reduced. Circulation had begun to be restored. I followed the rehabilitation exercises Natasha suggested which helped further improve the movement and circulation."

Chevonne, age 30, Computer technician and mother of 3

Lower back pain

"BIG thank you to Natasha. Being a basketball player there is a lot of wear and tear on the body apart from picking up some knocks and bruises. I had tightness in my lower back and explained the discomfort to Natasha. Over the weeks we went through a series of massages and stretches. In addition she also showed me stretches that could improve my posture in the long term. I very much appreciate her expertise, it has really help me and my team."

Bode, age 26, Competitive Basket ball player

Pre-Event 'London to Brighton Cycle Ride' Treatment

"Thanks to Natasha, the tension from training in my quads and calf muscles was greatly relieved in the sessions we had before I did the major event ride."

Millie, age 34, Reflexologist

Lower Back Pain

"Following a muscle strain across my lower back after a particularly strenuous rowing session, Natasha quickly and expertly assessed my symptoms, looking at how my body functioned as a whole to treat the source of pain. Natasha also gave me some great follow up exercises to strengthen my core and improve my range of movement."

Tom, age 41, Competitive rower`,
  },
  "/what-is-what-are-the-benifits": {
    parent: "therapy",
    headings: ["What are the Benefits?"],
    copy: `What is Sport Therapy?

What are the Benefits?

Sport therapy provides relief from musculoskeletal pain and dysfunction through the use of various massage techniques.

Advanced techniques can help alleviate the symptoms of conditions such as:

- Asthma
- Adhesive Capsulitis (frozen shoulder)
- Pregnancy related issues
- Irritable Bowel Syndrome
- Carpel Tunnel Syndrome
- Raynauds Syndrome
- Repetitive Strain Injury
- Tension headaches
- Migraines
- Sinusitis

Postural improvement

Improve common conditions such as back and neck pain

Reduce muscular tension and pain

Injury prevention/faster recovery from injuries

Improve flexibility and range of movement.

Enhanced training, performance and faster recovery

Realigning and dissipating scar tissue after surgery or injury

Alleviate stress`,
  },
  "/treatment": {
    parent: "therapy",
    headings: ["Common Injuries We Treat"],
    copy: `Techniques

We use techniques including:

- Deep tissue massage
- Muscle energy techniques (MET)
- Soft Tissue Release (STR)
- Deep friction techniques
- Neuro Muscular Technique (NMT)
- Connective Tissue Manipulation: Myofascial work releases tension and stress in fascia, allowing underlying muscles to move freely.
- Positional Release (PR)
- Manual mobilisation for joints and soft tissue

These techniques are often combined with exercises to stretch and strengthen muscle following treatment for you to do at home to continue the beneficial effects of the treatment.

Common Injuries We Treat

We treat a wide range of injuries including:

Recurring postural pain

Lower back injuries/pain/ache/sciatica

Upper back/neck problems, headaches and whiplash

Thoracic tightness/pain

Acute/chronic pain

Hip/groin problems

Knee, ankle and upper limb problems

Soft tissue rehabilitation

Joint stiffness

Muscular aches/pain/strains

Ligament sprains

Tendonitis

Pregnancy related back pain.

Release of restriction from a scar pre and post surgery

We take professional referrals from GP’s, consultants, physiotherapist’s, osteopath’s and podiatrist’s.

Whatever your injury or postural pain we can help you - just contact us`,
  },
  "/myofascial-release": {
    parent: "therapy",
    headings: [
      "What is Myofascia?",
      "Myofascial Release",
      "What does a Myofascial Release treatment feel like?",
    ],
    copy: `What is Myofascial Release?

Myofascial Release is a very effective hands-on therapy that has been used succesfully for over a hundred years to treat difficult and recurring injuries and pain.

What is Myofascia?

Myofascia is the dense tissue that surrounds and covers all your muscles and bones. This outer covering, called fascia is very strong and very flexible.

Myofascia is like a superficial body suit, which allows us to move freely, breathe properly and perform our daily tasks pain free. It spans the whole body and is totally connected as one piece of material. "It is called the tissue of movement".

In the normal healthy state the Myofascia is relaxed, soft and helps to maintain good posture, range of motion and flexibility. It also gives our bodies tremendous strength and helps us deal with excessive stress and injuries. Myofacia has the ability to stretch and move without restriction. When you experience physical trauma or inflammation Myofascia loses is pliability. It can become tight, restricted and a source of tension throughout the rest of the body. Trauma, such as a fall whiplash, surgery or habitual poor posture has a cumulative effect over time.

However, there is a treatment that can effectively treat this tough, tight fascial tissue making it more relaxed, pliable and soft.

Myofascial Release

Myofascial Release is an effective hands-on therapy which can directly change and improve health of the fascia. The purpose of Myofascial Release is to break down scar tissue and relax the muscle, restoring good posture.

Myofascial Release techniques focus on relaxing the deep tissue of the body providing lasting and effective relief.

This technique is applied directly on the body and uses slow and sometimes deep pressure to restore the proper health of the fascia.

Myofascial Release therapy can help alleviate:

Lower back pain

Headaches

Neck stiffness

Shoulder injuries including Adhesive Capsulitis (frozen shoulder)

Arthritic conditions

Sports injuries

Carpel Tunnel Syndrome

Postural pain

Respiratory conditions including Asthma

The benefits of using Myofascial Release therapy includes:

Reduction in muscle spasms

Improved joint movement

Decreased muscle and fascial tension

Improved breathing

Reduction in chronic recurring injuries

In general Myofascial Release is used to improve the health of your muscles and fascia, improving circulation and helping to restore good posture.

What does a Myofascial Release treatment feel like?

The pressure can range from very gentle touch to deeper pressure. The pressure from Myofascial Release therapy should never be beyond your tolerance and it is important to give feedback during the treatment.

Some patients may experience a slight skin burning sensation in the skin, which is perfectly normal and safe. Others may feel a gentle to deep stretch on the area being treated.

The techniques of Myofascial Release therapy help to re-organize and lengthen the tight tissue allowing for better movement and health to the tissue.

Myofascial Release was specifically designed to relax the fascia throughout the whole body. Tension in fascia can effect and cause pain in other areas and we may treat other areas of the body to positively effect your treatment outcome.`,
  },
  "/what-to-expect": {
    parent: "therapy",
    headings: ["What To Wear"],
    copy: `What To Expect

At your first appointment I will:

Take a brief medical history

Assess your injury/area of pain

Assess your posture, the joint and surrounding area

Your goals we are working towards and a treatment plan will be discussed which may include home exercises. Every treatment is tailored to the individual clients needs.

Feedback is essential following each treatment to review progression and amend your treatment plan if necessary.

What To Wear

If possible please wear loose comfortable clothing or bring shorts with you.

Depending on the location of your injury you may be required to remove some items of clothing.

However, treatment can be done through clothes if you prefer.

You don’t have to be an athlete to use NJH Sports Therapy & Pilates. I get as much satisfaction from making a difference to the life of a patient with a disabling condition as I do working with elite sportsmen and women.`,
  },
  "/kinesiology-taping": {
    parent: "therapy",
    images: [
      {
        src: "/images/legacy/kinesiology-taping.jpg",
        alt: "Examples of kinesiology taping",
      },
    ],
    headings: [
      "- Symptom Reduction Taping:",
      "- Oedema Control Taping:",
      "- Postural Control Taping:",
      "- Power Taping:",
    ],
    copy: `Kinesiology Taping

This is a highly beneficial treatment aiding pain relief, reducing swelling and posture control.

There are four different types of taping available:

- Symptom Reduction Taping:

helping reduce muscular pain and support joints.

- Oedema Control Taping:

helping reduce acute or chronic swelling.

- Postural Control Taping:

to improve awareness of conditions such as protracted shoulders,
thoracic kyphosis, internal rotation of the shoulder,
pelvic alignment and lower limb stability.

- Power Taping:

to help treat pain, improve flexibility and reduce imbalances.

Taping can be included as part of your appointment time or as a stand alone session of approximately 15-20 minutes.

If you would like more information please do not hesitate to contact me on 07881 821901`,
  },
  "/office-based-sports-massage": {
    parent: "therapy",
    copy: `Office Based Sports Massage

NJH Sports Therapy also offers office based treatments.

With busy work schedules, finding time for yourself can be hard. However, we offer on-site massage treatments, carried out in the privacy of your workplace meeting room or other quiet areas within your office.

This offers clients the ideal solution – addressing injuries, whilst enabling you to fulfil busy work commitments.

This also offers employers an important opportunity to ensure their staff are in the best health possible to fulfil their roles, reducing sickness time and absence from the workplace. It can also be a great Christmas bonus or gesture of good will for working extra hours to meet a tight deadline.`,
  },
  "/about": {
    heroTitle: "About",
    images: [
      {
        src: "/images/legacy/natasha-treatment.png",
        alt: "Natasha Hadland working with a client",
      },
    ],
    copy: `LSSM Diploma in Sport & Remedial Massage

BTEC Level 5 Professional Diploma in Clinical Sport & Remedial Massage

VTCT Level 3 Diploma in Anatomy, Physiology and Pathology

Certified STOTT Pilates Instructor REPS registered Level 3

Natasha Hadland

After a ten year career in the Fashion industry, I took a career break to spend time with my young family.

Time away from full time work enabled me to pursue my passion for sports and my interest in human anatomy and physiology. This led to a new and exciting career as a Sports Therapist after completing a Diploma at the London School of Sports Massage, which has been the pioneer and leader in Sport and Remedial Massage training since 1989. My interest in rehabilitation and postural issues then led onto my training as a Certified STOTT Pilates instructor.

I am passionate about improving clients' health, fitness and well being. Through the use of many advanced Soft Tissue techniques and Pilates exercises I can deliver specific and effective treatment for your injury or postural issues. I aim to provide the highest level of professional and personal care to you at all times.

CONTACT US`,
  },
  "/prices": {
    parent: "contact",
    headings: [
      "Treatment Duration:",
      "Treatment Charges:",
      "Pilates Charges 1 hour session:",
    ],
    copy: `CONTACT US

Treatment Duration:

Most sessions last approximately 1 hour.

However treatment durations can vary at the discretion of your therapist to avoid over treatment in certain cases.

You may be recommended 30 minute appointments if this is more appropriate to your individual condition.

If you are a new patient you may be recommended 90 minutes based on the complexity of your case.

Treatment duration for new patients will be discussed and agreed
booking or consultation.

Treatment Charges:

£130 for up to 90 minutes

£85 standard treatment session for up to 1 hour

£60 for up to 30 minutes

Sunday and Bank Holiday appointments are available with an additional £10 surcharge.

Pilates Charges 1 hour session:

£85 for 1:1 Pilates

£95 2:1 shared with a friend/partner

£85 initial 1:1 prior to joining Small Group Pilates Sessions

£21 per Small Group Pilates session (paid for in termly blocks)`,
  },
  "/price-list": {
    parent: "contact",
    headings: [
      "Sports Therapy Treatment Charges:",
      "Pilates Charges 1 hour session:",
    ],
    copy: `Price List

Sports Therapy Treatment Charges:

£130 to 90 minutes

£85 standard treatment session for up to 1 hour

£60 for up to 30 minutes

Sunday and Bank Holiday appointments are available with an additional £10 surcharge.

Pilates Charges 1 hour session:

£85 for 1:1 Pilates

£95 2:1 shared with a friend/partner

£85 initial 1:1 prior to joining Small Group Pilates Sessions

£21 per Small Group Pilates session (Once payment is made your place is reserved for the entire block. Please note no refunds will be given.)`,
  },
};
