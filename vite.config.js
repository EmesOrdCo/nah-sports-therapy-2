...
// GitHub Pages serves the build from https://haza1410.github.io/njh-sports-therapy/.
...
export default defineConfig(({ command, isPreview }) => ({
  base:
    command === "build" || isPreview
      ? (process.env.BASE_PATH ?? "/njh-sports-therapy/")
      : "/",
  plugins: [
    {
      // GitHub Pages has no server-side rewrites, so deep links such as
      // /njh-sports-therapy/pilates need a 404 fallback that boots the same app.
